...
class Faceformer(nn.Module):
    ...
    def forward(self, audio, template, vertice, one_hot, criterion, teacher_forcing=True):
        ...
        :param template:
        :param vertice:
        :param one_hot: one_hot tensor([[0., 0., 1., 0., 0., 0., 0., 0.]], device='cuda:0')
        :param criterion:
        ...
        # print("vertice:", vertice.shape) # ([1, 121, 15069])  ([1, 140, 15069])
        # print("vertice:", vertice)  # 就是正常的原始顶点的格式
        ...
        if teacher_forcing:
            ...
            for i in range(frame_num):
                ...
                vertice_out = self.transformer_decoder(vertice_input, hidden_states, tgt_mask=tgt_mask,
                                                       memory_mask=memory_mask)
                vertice_out = self.vertice_map_r(vertice_out)  # 预测得到的是运动向量，e-03，e-04，e-05级别
    ...
    def predict(self, audio, template, one_hot):
        ...
        for i in range(frame_num):
            ...
            # biased casual mh self-attention
            tgt_mask = self.biased_mask[:, :vertice_input.shape[1], :vertice_input.shape[1]].clone().detach().to(
                device=self.device)
